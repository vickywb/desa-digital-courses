...
onMounted(async () => {
    try {
        const res = await client.get(`/village-staff/head-families/${route.params.id}`);
...
<template>
    <div class="flex flex-col gap-[14px]">
        <div class="flex items-center gap-2">
            <router-link :to="`/staff/head-families/${route.params.id}/members`"
            ...
            <span class="font-medium text-desa-secondary">Detail Kepala Keluarga</span>
        </div>
            ...
            <div class="w-full lg:w-[400px] shrink-0 rounded-2xl bg-white p-4 sm:p-6 flex flex-col gap-6 h-fit">
                <div class="flex flex-col items-center gap-4">
                    <div class="flex size-24 sm:size-28 rounded-full overflow-hidden bg-desa-foreshadow">
                        <img :src="item.profile_picture?.url ?? '/desa-digital/src/assets/images/no-image.png'"
                            class="w-full h-full object-cover" alt="photo" />
                    </div>
                    <div class="text-center">
                        <h2 class="font-semibold text-xl leading-6">{{ item.full_name }}</h2>
                        <p class="font-medium text-sm text-desa-secondary mt-1">Kepala Keluarga</p>
                    </div>
                </div>
