                ...
                <table class="w-full text-left">
                    ...
                    <tbody>
                            ...
                            <td class="px-4 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="flex size-10 rounded-full overflow-hidden bg-desa-foreshadow shrink-0">
                                        <img :src="item.profile_picture?.url ?? '/desa-digital/src/assets/images/no-image.png'" class="w-full h-full object-cover" alt="photo">
                                    </div>
                                    <span class="font-semibold leading-5">{{ item.full_name }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-4 text-desa-secondary">{{ item.identity_number }}</td>
                            <td class="px-4 py-4">{{ genderLabel(item.gender) }}</td>
                        ...
                        <tr v-if="!items.length">
                            <td colspan="6" class="px-4 py-12 text-center text-desa-secondary font-medium">No head of family data</td>
                        </tr>
                    </tbody>
                </table>
            </div>
