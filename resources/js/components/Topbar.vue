...
<template>
    <div id="Top-Bar" class="relative flex h-[72px] md:h-[116px] shrink-0">
        ...
        >
            ...
            >
                <svg
                    class="size-5 text-desa-secondary"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    stroke-width="2"
            ...
            <div class="flex items-center gap-2 md:gap-4">
                <div
                    class="flex size-10 md:size-14 shrink-0 rounded-full overflow-hidden bg-desa-foreshadow"
                >
                    <img
                        ...
                        v-if="
                            user?.role === 'admin' ||
                            user?.role === 'head_village' ||
                            user?.role === 'staff'
                        "
                    />
                    <img
                        :src="user?.head_of_family?.profile_picture?.url"
                        class="w-full h-full object-cover"
                        alt="photo"
                        v-else
                    />
                ...
                <button
                    @click="logout"
                    class="flex size-5 md:size-6 shrink-0 cursor-pointer"
                >
                    <img
